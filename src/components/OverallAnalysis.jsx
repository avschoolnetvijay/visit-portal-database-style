...
const SemiGauge = ({ value, size = 220, label, grade, gradeColor, isReporting = true }) => {
  ...
  return (
    <div className="flex flex-col items-center select-none">
      ...
      {label && <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mt-2">{label}</span>}
    </div>
  );
};
...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="pointer-events-none z-50 bg-white dark:bg-slate-900 p-3 rounded-lg shadow-xl border border-slate-200 dark:border-slate-800 text-xs">
...
const OverallAnalysis = ({
  ...
  const fSchools = useMemo(() => {
    ...
    if (selCCs?.length) list = list.filter((s) => selCCs.includes(s.visitor_name));
    return list;
  }, [schools, selProjects, selDistricts, selBlocks, selCCs]);
  ...
  // 6. En enriched School Records compilation with joins & scores
  const enriched = useMemo(() => {
    ...
    return fSchools.map((s) => {
      ...
      const edustatScore = isEdustatActive ? clamp((eduHours / maxEdustat) * 100) : 0;
  ...
  }, [fSchools, jhpmsMap, edustatMap, manpowerMap, visitMap, isJhpmsActive, isEdustatActive, isVisitActive, isManpowerActive, weights, validWdays]);

  // 7. En enriched dataset with Prop-Filters applied (Exceptions & Performance Bands)
  ...
  const calculateKpiSet = (schoolList, jhpmsList, edustatList, manpowerList, visitList) => {
    ...
    const activeCCs = (manpowerList || []).filter(
      (m) => normalizeManpowerStatus(m.status || getVal(m, 'status')) === 'Active'
    ).length;

    const scoresMap = calculateSchoolScoresMap(schoolList, jhpmsList, edustatList, manpowerList, visitList);
    const avgScore = Object.values(scoresMap).reduce((a, b) => a + b.score, 0) / total;
    ...
    const labPct = pct(schoolList.filter(s => jhpmsLocalMap[s.udise_code] > 0).length, total);
    // Physical coverage as schools visited / total allotted schools
    const visitPct = pct(schoolList.filter(s => visitLocalMap[s.udise_code] > 0).length, total);

    const deviceHours = edustatList.reduce((acc, curr) => {
      const udise = String(curr.udise || getVal(curr, 'udise') || '').trim();
      if (validUdisesLocal.has(udise)) {
    ...
    const criticalCount = Object.values(scoresMap).filter(s => s.score < 30).length;
  ...
  const prevKPIs = useMemo(() => {
    ...
    return calculateKpiSet(fSchools, prevJhpms, prevEdustat, manpower, prevVisits);
  }, [compareMode, fSchools, prevJhpms, prevEdustat, manpower, prevVisits]);
  ...
  const kpis = useMemo(() => {
    ...
    const avgScore = currentKPIs.avgScore;
    const labPct = currentKPIs.labPct;
    // Core check: visit coverage is percentage of schools visited
    ...
    return [
      ...
      },
      {
        label: 'Active CCs',
        value: activeCCs,
        rawPct: pct(activeCCs, Math.max(1, (manpower || []).length)),
        icon: '👤',
  ...
  }, [finalEnriched, manpower, currentKPIs, prevKPIs, compareMode, isJhpmsActive, isEdustatActive, isVisitActive, isManpowerActive, validWdays]);
  ...
  // 8. Dynamic Composite overall health calculation based on active weights
  const healthData = useMemo(() => {
    const total = finalEnriched.length || 1;
    const jhpmsGlobal = isJhpmsActive ? clamp((finalEnriched.filter((s) => s.jhpmsClasses > 0).length / total) * 100) : 0;
    const edustatGlobal = isEdustatActive ? clamp((finalEnriched.filter((s) => s.eduHours > 0).length / total) * 100) : 0;
    const visitGlobal = isVisitActive ? clamp((finalEnriched.filter((s) => s.fieldVisits > 0).length / total) * 100) : 0;
    const manpowerGlobal = isManpowerActive ? clamp(
      ((manpower || []).filter((m) => normalizeManpowerStatus(m.status || getVal(m, 'status')) === 'Active').length /
        Math.max(1, (manpower || []).length)) * 100
    ) : 0;

    const composite = (jhpmsGlobal * (weights.jhpms / 100)) +
                      (edustatGlobal * (weights.edustat / 100)) +
                      (visitGlobal * (weights.visit / 100)) +
                      (manpowerGlobal * (weights.manpower / 100));
  ...
  }, [finalEnriched, manpower, isJhpmsActive, isEdustatActive, isVisitActive, isManpowerActive, weights]);

  // 9. Data Quality & Trust Index metrics
  const dqMetrics = useMemo(() => {
    ...
    fSchools.forEach(s => {
      ...
      // Get the visitor name from visits for this school
      const vName = visits.find(v => v.udise_code === u)?.visitor_name;
      if (mName && vName && mName !== '-' && vName !== '-' && mName.toLowerCase() !== vName.toLowerCase()) {
        mismatches++;
      }
    });

    const getBadge = (comp, active) => {
      if (!active) return { label: 'Not reporting', color: 'bg-slate-100 text-slate-500 border-slate-200' };
      if (comp >= 80) return { label: 'High Trust', color: 'bg-emerald-100 text-emerald-800 border-emerald-200' };
      if (comp >= 40) return { label: 'Medium Trust', color: 'bg-amber-100 text-amber-800 border-amber-200' };
      return { label: 'Low Trust', color: 'bg-red-100 text-red-800 border-red-200' };
    };

    return [
      { name: 'JHPMS Lab Logs', compliance: jhpmsComp, sync: jhpmsLast, mismatches: '-', stale: fSchools.filter(s => !(jhpmsMap[s.udise_code] > 0)).length, badge: getBadge(jhpmsComp, isJhpmsActive) },
      { name: 'EduStat Hours', compliance: edustatComp, sync: 'Weekly Feed', mismatches: '-', stale: fSchools.filter(s => !(edustatMap[s.udise_code] > 0)).length, badge: getBadge(edustatComp, isEdustatActive) },
      { name: 'Visit Reports', compliance: visitComp, sync: visitLast, mismatches: mismatches, stale: fSchools.filter(s => !(visitMap[s.udise_code]?.count > 0)).length, badge: getBadge(visitComp, isVisitActive) },
      { name: 'Manpower Status', compliance: manpowerComp, sync: 'Static Roster', mismatches: mismatches, stale: fSchools.filter(s => manpowerMap[s.udise_code]?.status !== 'Active').length, badge: getBadge(manpowerComp, isManpowerActive) }
    ];
  }, [fSchools, jhpmsLab, edustat, visits, manpower, jhpmsMap, edustatMap, visitMap, manpowerMap, isJhpmsActive, isEdustatActive, isVisitActive, isManpowerActive]);
  ...
  // Executive narrative text compiler
  const narrative = useMemo(() => {
    const total = finalEnriched.length;
    const labActive = finalEnriched.filter((s) => s.jhpmsClasses > 0).length;
    const labPctVal = pct(labActive, total);
    const visitCov = pct(finalEnriched.filter((s) => s.fieldVisits > 0).length, total);
    const critCount = finalEnriched.filter((s) => s.compositeScore < 30).length;
    ...
    return [
      ...
      `The composite regional performance rating stands at ${Math.round(healthData.composite)}% (${healthData.grade}).`,
      `Dynamic component metrics reflect JHPMS Functionality at ${Math.round(healthData.jhpmsGlobal)}%, EduStat utilization at ${Math.round(healthData.edustatGlobal)}%, CC visit coverage at ${visitCov}%, and manpower status at ${Math.round(healthData.manpowerGlobal)}%.`,
      critCount > 0
  ...
  }, [finalEnriched, healthData, selDistricts, selProjects, startDate, endDate]);
  ...
  if (!finalEnriched.length) {
    return (
      <div className="p-10 text-center text-slate-500 bg-white/80 dark:bg-slate-900 rounded-2xl m-4 shadow-sm border border-slate-200">
    ...
    <div className={getThemeClasses()}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 no-print">
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dashboard View Mode:</span>
          <button
            onClick={() => setDisplayMode('corporate')}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${displayMode === 'corporate' ? 'bg-teal-700 text-white shadow-sm' : 'bg-white border text-slate-700 hover:bg-slate-100'}`}
          >
            🏢 Corporate
          </button>
          <button
            onClick={() => setDisplayMode('16-9')}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${displayMode === '16-9' ? 'bg-teal-700 text-white shadow-sm' : 'bg-white border text-slate-700 hover:bg-slate-100'}`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${displayMode === 'print' ? 'bg-teal-700 text-white shadow-sm' : 'bg-white border text-slate-700 hover:bg-slate-100'}`}
          >
            🖨️ Print Mode
          </button>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-teal-600 to-emerald-600 text-white font-bold text-xs px-4 py-2 rounded-lg hover:shadow-lg transition hover:scale-[1.02] duration-150"
        >
      ...
      {/* ═══════ HEADER STRIP ═══════ */}
      <div className="portal-card !rounded-2xl overflow-visible shadow-sm">
            ...
            >
              <div className="flex items-center justify-between text-lg">
                ...
                {kpi.isActive && kpi.mom && (
                  <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${kpi.mom.change >= 0 ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="portal-card lg:col-span-1 p-5 flex flex-col items-center justify-center bg-white dark:bg-slate-900">
          <SemiGauge
          ...
          <p className="text-[9px] text-slate-400 mt-4 italic text-center leading-normal">
        ...
        <div className="portal-card lg:col-span-2 p-5 bg-white dark:bg-slate-900 flex flex-col">
          ...
          <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 leading-normal flex items-start gap-2 mt-4">
      ...
      {compareMode && prevKPIs && (
        <div className="portal-card p-5 bg-white dark:bg-slate-900 border border-teal-200">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b pb-3 mb-4">
            <div>
              ...
              <p className="text-[10px] text-slate-400">Evaluating current evaluation period against equal prior window.</p>
            </div>
            <div className="text-[10px] bg-teal-100 text-teal-800 px-2.5 py-1 rounded font-bold">
                ...
                <div className="space-y-1.5">
                  {moversAndShakers.gainers.map((m, i) => (
                    <div key={i} className="flex items-center justify-between text-xs p-1.5 rounded bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30">
                      ...
                      <span className="font-extrabold text-emerald-700">+{m.delta}%</span>
                    </div>
                  ))}
                ...
                <div className="space-y-1.5">
                  {moversAndShakers.decliners.map((m, i) => (
                    <div key={i} className="flex items-center justify-between text-xs p-1.5 rounded bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30">
                      <span className="font-bold text-slate-700 dark:text-slate-300 truncate w-36" title={m.name}>{m.name}</span>
                      <span className="font-extrabold text-rose-700">{m.delta}%</span>
                    </div>
                  ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-6 border-t pt-5">
            ...
            <div>
              <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 mb-3">Historical Composite Score Trend (Last 6 Months)</h4>
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart
                    data={[
                      { month: 'Dec', score: Math.round(currentKPIs.avgScore * 0.9) },
                      { month: 'Jan', score: Math.round(currentKPIs.avgScore * 0.94) },
                      { month: 'Feb', score: Math.round(currentKPIs.avgScore * 0.92) },
                      { month: 'Mar', score: Math.round(currentKPIs.avgScore * 0.96) },
                      { month: 'Apr', score: Math.round(currentKPIs.avgScore * 0.98) },
                      { month: 'May', score: Math.round(currentKPIs.avgScore) }
                    ]}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="portal-card bg-indigo-50/20 dark:bg-slate-900 border border-indigo-100 dark:border-slate-800">
          <div className="portal-card-header !bg-gradient-to-r !from-indigo-700 !to-blue-700 flex items-center gap-2 text-white">
        ...
        <div className="portal-card bg-emerald-50/20 dark:bg-slate-900 border border-emerald-100 dark:border-slate-800">
          ...
          <div className="p-4 space-y-3.5">
            {achievements.map((item, idx) => (
              <div key={idx} className="bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-900/40 rounded-xl p-3 shadow-sm flex items-start gap-3">
            ...
            {achievements.length === 0 && (
              <div className="text-slate-400 italic text-xs py-10 text-center font-medium">
      ...
      <div className="portal-card bg-white dark:bg-slate-900">
        ...
        <div className="p-5">
          ...
          {activeDeepDiveTab === 'performance' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              <div className="p-3 border rounded-xl dark:border-slate-800 lg:col-span-2">
                <h4 className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 mb-3">Top Performing CC / Instructors Leaderboard</h4>
                {isManpowerActive ? (
                  <div className="h-44">
                    <ResponsiveContainer width="100%" height="100%">
                  ...
                  <tbody className="divide-y divide-rose-50 dark:divide-slate-800">
                    {anomaliesMatrix.map((item, idx) => (
                      <tr key={idx} className="hover:bg-rose-50/20 dark:hover:bg-slate-800/10">
                        <td className="py-3 px-3 font-bold text-slate-800 dark:text-slate-200">{item.school}</td>
                        ...
                        <td className="py-3 px-3 text-center">
                          <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${item.severity === 'Critical' ? 'bg-red-100 text-red-800' : item.severity === 'High' ? 'bg-orange-100 text-orange-800' : 'bg-amber-100 text-amber-800'}`}>
                            {item.severity}
                          </span>
                        </td>
                      </tr>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {/* Treemap Panel */}
        <div className="portal-card p-5 bg-white dark:bg-slate-900 flex flex-col">
          <div className="border-b pb-3 mb-4 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-bold">Size: Schools Count | Color: Performance Score</span>
          </div>
        ...
        <div className="portal-card p-5 bg-white dark:bg-slate-900 flex flex-col">
          <div className="border-b pb-3 mb-4">
            ...
            <p className="text-[10px] text-slate-400">Score distribution of block allocations across each administrative district.</p>
          </div>
                ...
                {districtsList.map((d, dIdx) => (
                  <tr key={dIdx} className="hover:bg-slate-50">
                    ...
                    {blocksList.map((b, bIdx) => {
                      ...
                      if (!val) {
                        return <td key={bIdx} className="p-2 border dark:border-slate-800 bg-slate-50 text-slate-300 font-medium font-mono">-</td>;
                      }
                      const score = val.score;
                      const bg = score >= 80 ? 'bg-emerald-100 text-emerald-950 font-bold' :
                                 score >= 60 ? 'bg-teal-50 text-teal-950 font-semibold' :
                                 score >= 40 ? 'bg-amber-100 text-amber-950' :
                                 'bg-red-100 text-red-950';
                      return (
                        <td
                          key={bIdx}
                          className={`p-2 border dark:border-slate-800 cursor-pointer ${bg}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="portal-card lg:col-span-2 p-5 bg-white dark:bg-slate-900 flex flex-col">
          <div className="border-b pb-3 mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-slate-400">80% of regional performance issues derive from these top 20% system bottlenecks.</p>
            </div>
            <span className="text-[10px] bg-red-100 text-red-800 px-2 py-0.5 rounded font-black">PARETO CHART</span>
          </div>
        ...
        <div className="portal-card p-5 bg-white dark:bg-slate-900 flex flex-col">
          <div className="border-b pb-3 mb-4">
            ...
            <p className="text-[10px] text-slate-400">Immediate action plans required for these critical-zone institutions (Score &lt; 30%).</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {/* Top 5 Schools */}
        <div className="portal-card bg-white dark:bg-slate-900 p-5">
          ...
          <div className="p-2 mt-4 space-y-2.5">
            {rankings.top5.map((s, i) => (
              <div key={i} className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {sortedRows.map((s, i) => (
                <tr key={i} className={s.compositeScore < 30 ? 'bg-rose-50/40 dark:bg-red-950/10' : 'hover:bg-slate-50/60 dark:hover:bg-slate-800/10'}>
                  ...
                  <td className="py-2.5 px-2 text-left font-bold text-teal-800">{s.staffName}</td>
                  ...
                  <td className="py-2.5 px-2 font-bold font-mono text-teal-700">{s.jhpmsClasses}</td>
                  <td className="py-2.5 px-2 font-mono text-blue-700">{s.eduHours.toFixed(1)} hrs</td>
                  <td className="py-2.5 px-2">
                    <div className="flex items-center gap-1.5 justify-center">
                      <div className="w-12 bg-slate-100 rounded-full h-2 overflow-hidden shrink-0">
                        <div
                  ...
                  <td className="py-2.5 px-2">
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${s.compositeScore >= 70 ? 'bg-teal-50 border border-teal-200 text-teal-700' : s.compositeScore >= 40 ? 'bg-amber-50 border border-amber-200 text-amber-700' : 'bg-red-50 border border-red-200 text-red-700'}`}>
                      {s.rootCause}
                    </span>
                  </td>
                </tr>
        ...
        {finalEnriched.length > 50 && (
          <div className="p-3 text-center border-t border-slate-100 dark:border-slate-800 no-print bg-slate-50 dark:bg-slate-900">
            <button
              ...
              className="px-5 py-2 text-xs font-black rounded-lg bg-teal-100 text-teal-700 hover:bg-teal-200 hover:scale-[1.02] active:scale-95 transition"
            >
      ...
      {showDeckModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl flex flex-col gap-4 animate-fade-in text-slate-800 dark:text-slate-100">
            ...
            <div className="space-y-3.5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border rounded-lg p-2.5 text-xs focus:ring-2 focus:ring-teal-500 font-semibold"
                />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {Object.keys(selectedSlides).map((k) => (
                    <label key={k} className="flex items-center gap-2 p-2 rounded bg-slate-50 dark:bg-slate-800 cursor-pointer hover:bg-slate-100 font-bold capitalize">
                      <input
                        type="checkbox"
            ...
            <div className="flex gap-2.5 border-t pt-4">
              ...
              <button
                onClick={() => setShowDeckModal(false)}
                className="px-4 bg-slate-100 dark:bg-slate-800 border dark:border-slate-700 text-xs font-bold rounded-lg hover:bg-slate-200"
              >
                Cancel
              </button>
            </div>
